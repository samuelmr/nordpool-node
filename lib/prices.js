...
Prices.prototype.at = function(dateTime, opts, callback) {
  var atTime = moment(dateTime)
  ...
  this.getValues(opts, function(error, results) {
    ...
    for (let i=0; i<results.length; i++) {
      ...
      if (date.get('dayOfYear') == atTime.get('dayOfYear') &&
          date.get('hour') == atTime.get('hour')) {
        callback(null, results[i])
        return
      }
    }
    callback(new Error('No results found for ' + moment.format()))
  })
}
...
Prices.prototype.getValues = function(opts, callback) {
  ...
  if (opts.startDate) {
    ...
    url += '&startDate=' + startDate.tz(config.timezone).format("DD-MM-YYYY")
  }
  if (opts.endDate) {
    endDate = moment(opts.endDate).hour(23).minute(59).second(59).millisecond(999)
    url += '&endDate=' + endDate.tz(config.timezone).format("DD-MM-YYYY")
  }
